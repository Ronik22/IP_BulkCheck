...
function exitProgram #Exit button functionality
{
    $bulkChecker.close()
}
...
function bulkCheck {
    ...
    Import-csv $csvPath -Header "IP" | ForEach {
        if($_.IP -as [ipAddress] -as [Bool]) { # Check if it's an IP Address
            Invoke-WebRequest -Uri ("https://api.abuseipdb.com/api/v2/check?ipAddress=$($_.IP)&maxAgeInDays=365&verbose=yes") -Headers @{'Accept' = 'application/json'; 'Key' = $ApiKey} -usebasicparsing | add-content -path $jsonTempPath
        } else {
    ...
    Get-content $jsonTempPath | ConvertFrom-Json | select -ExpandProperty Data | select ipAddress, abuseConfidenceScore, isp, domain, usageType, countryCode, countryName, totalReports, lastReportedAt, numDistinctUsers, isTor | ConvertTo-CSV -NoTypeInformation | add-content $exportPath #Create CSV output file       
